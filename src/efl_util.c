...
typedef struct _Efl_Util_Wl_Surface_Scr_Md_Info
{
   void *surface; /* wl_surface */
   unsigned int mode;
   Eina_Bool wait_for_done;
} Efl_Util_Wl_Surface_Scr_Md_Info;
...
static void                    _cb_wl_tz_win_scr_md_done(void *data, struct tizen_window_screen *proto, struct wl_surface *surface, uint32_t mode, uint32_t state);
...
struct tizen_window_screen_listener _wl_tz_win_scr_md_listener =
{
   _cb_wl_tz_win_scr_md_done
};
#endif /* end of WAYLAND */
   ...
   if (!strcmp(interface, "tizen_notification"))
   ...
   else if (!strcmp(interface, "tizen_window_screen"))
     {
        ...
        tizen_window_screen_add_listener(proto,
                                         &_wl_tz_win_scr_md_listener,
                                         NULL);

        _eflutil.wl.win_scr_md.hash = eina_hash_pointer_new(free);
        _eflutil.wl.win_scr_md.proto = proto;
     }
}
                         ...
                         unsigned int name)
{
   ...
   _eflutil.wl.win_scr_md.proto = NULL;
   eina_hash_free(_eflutil.wl.win_scr_md.hash);
}
...
static void
_cb_wl_tz_win_scr_md_done(void *data,
                          struct tizen_window_screen *proto,
                          struct wl_surface *surface,
                          uint32_t mode,
                          uint32_t state)
{
   ...
   Efl_Util_Wl_Surface_Scr_Md_Info *scr_md_info;
   Efl_Util_Callback_Info *cb_info;

   scr_md_info = eina_hash_find(_eflutil.wl.win_scr_md.hash, &surface);
   if (scr_md_info)
     {
        scr_md_info->mode = mode;
        scr_md_info->wait_for_done = EINA_FALSE;
     }
   ...
   if (wlwin)
     {
        ...
        while (!_eflutil.wl.win_scr_md.proto)
          wl_display_dispatch(_eflutil.wl.dpy);
        ...
        if (!scr_md_info)
          {
             scr_md_info = calloc(1, sizeof(Efl_Util_Wl_Surface_Scr_Md_Info));
             EINA_SAFETY_ON_NULL_RETURN_VAL(scr_md_info, EFL_UTIL_ERROR_OUT_OF_MEMORY);

             scr_md_info->surface = surface;
             scr_md_info->mode = (unsigned int)mode;
             scr_md_info->wait_for_done = EINA_TRUE;

             eina_hash_add(_eflutil.wl.win_scr_md.hash,
                           &surface,
                           scr_md_info);
          }
        else
        ...
        tizen_window_screen_set_mode(_eflutil.wl.win_scr_md.proto,
                                     surface,
                                     (unsigned int)mode);
   ...
   if (wlwin)
     {
        ...
        if (scr_md_info)
          {
             if (scr_md_info->wait_for_done)
               {
                  while (scr_md_info->wait_for_done)
                    {
                       ecore_wl_flush();
                       wl_display_dispatch(_eflutil.wl.dpy);
                    }
